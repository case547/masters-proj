...
def main(folder, csv_title, num_seconds, delta_t=1):
    """Run the simulation, recording arrivals and tyre PM."""
    ...
    with open(os.path.join(folder,f"{csv_title}_{delta_t}s.csv"), "w", newline="") as f:
        csv_writer = csv.writer(f)
        csv_writer.writerow(["sim_time","total_arrived","tyre_pm","tyre_pm_cumulative"])

    assert num_seconds % delta_t == 0
    num_steps = num_seconds / delta_t
    ...
    for step in range(int(num_steps)):
        ...
        # Log to CSV
        with open(os.path.join(folder,f"{csv_title}_{delta_t}s"), "a", newline="") as f:
            csv_writer = csv.writer(f)
            sim_time = step * delta_t  # can also use traci.simulation.getTime()
            csv_writer.writerow([sim_time, total_arrived, tyre_pm, tyre_pm_cumulative])

        # Log to TensorBoard
        tb_writer.add_scalar("total_arrived", total_arrived, step)
...
if __name__ == "__main__":
    ...
    main(folder=p.parent, csv_title="results", num_seconds=traci.simulation.getEndTime(), delta_t=1)
